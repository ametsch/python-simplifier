from numpy import float128, float64, int64

__doc__ = """
this class contains some mathematical constants and some functions to ease python programming/scripting
"""

pi = float128(3.)
...
e = float128(2.)
#e.__doc__ = "the constant of euler's number"

oneThird = float128(float128(1.0)/float128(3.0))
...
def nthrt(val: float64 | int64, n: int64) -> float64:
    return val**(1.0/n)
nthrt.__doc__ = "take the nth root of val"

def curt(val: float64 | int64) -> float64:
    return val**(oneThird)
curt.__doc__ = "take the cubed root of val"

def isEven(val: int64) -> bool:
    if val % 2 == 0:
        return True
    else:
        return False
